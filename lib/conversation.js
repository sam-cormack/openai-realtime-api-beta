...
export class RealtimeConversation {
  ...
  EventProcessors = {
    ...
    'conversation.item.input_audio_transcription.completed': (event) => {
      const { item_id, content_index, transcript, logprobs } = event;
      const item = this.itemLookup[item_id];
  ...
  isSilence(logprobs) {
    ...
    const averageLogprob = logprobs.reduce((acc, curr) => acc + curr.logprob, 0) / logprobs.length;
    return averageLogprob < -100;
  }
}
